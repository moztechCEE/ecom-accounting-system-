...
59 bilingual entries cover 9 navigation groups and 70 route or filter entry points, including all 16 configured WMS portal destinations. The root route is a role-dependent redirect. Employee login/reset routes are aliases of the account-security guide; the separate customer login and nested procurement routes have dedicated B2B guides. The parameterized warehouse route is expanded from the reviewed WMS and report configuration.

59 篇雙語指南涵蓋 9 個導覽群組及 70 個路由或篩選入口，含全部 16 個 WMS 入口。根路由依角色導向；員工登入與重設密碼列為帳號安全指南入口，獨立的客戶登入與巢狀採購路由另有 B2B 指南。動態儲運路由依已審閱的 WMS 與報表設定展開。
...
- Customer B2B access uses a separate session and company scope. A submitted request and login-protected quote link do not reserve stock; staff review each line manually before confirming a sales order. Supplier account records exist, but supplier login and customer self-service password reset are not available. / 客戶 B2B 入口使用獨立登入及公司隔離；送出需求與受保護的報價連結不預留庫存。員工逐行人工核庫後才能確認銷單；供應商帳號可建檔，但供應商登入與客戶自助重設密碼尚未提供。
