...
import { buildRequestInput, canConfirmRequest, canIssueQuote, formalQuotePath, quotePath, statusText } from '../src/pages/b2b/order.ts'
...
test('formal quote requires full stock review; sales order requires explicit acceptance', () => {
  const base = { status: 'stock_confirmed' as const, quoteVersion: null, quoteStatus: null as 'sent' | 'accepted' | null, salesOrderId: null, items: [{ quantity: 2, confirmedQuantity: 2 }] }
