...
export function wmsFixture():Plugin {
  ...
  return {name:'local-wms-fixture',configureServer(server){server.middlewares.use(async(req,res,next)=>{
    ...
    if(!['admin','business','worker','picker','packer','dispatcher','shipping'].includes(requestedRole))return send(403,{})
    ...
    try {
      ...
      if(url.pathname.endsWith('/stations'))return send(200,requestedRole==='admin'?['dispatch','pick','pack','shipping']:requestedRole==='worker'?['pick','pack']:[({picker:'pick',packer:'pack',dispatcher:'dispatch',shipping:'shipping'} as Record<string,string>)[role]])
