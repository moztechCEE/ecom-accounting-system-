...
export default function B2bWorkbenchPage() {
  ...
  const customerName = (id: string | null) => setup?.customers.find((item) => item.id === id)?.companyName || setup?.customers.find((item) => item.id === id)?.name || id || '—'
  ...
  return <div className="page-section-stack" style={{ maxWidth: 1500, margin: '0 auto', padding: '10px 4px 50px' }}>
    ...
    <Modal title="商品發布設定" open={catalogOpen} confirmLoading={saving} onCancel={() => setCatalogOpen(false)} onOk={() => void saveCatalog()} okText="儲存" destroyOnHidden><Form form={catalogForm} layout="vertical"><Form.Item name="productId" label="商品" rules={[{ required: true, message: '請選擇商品' }]}><Select showSearch optionFilterProp="label" options={setup?.products.map((item) => ({ value: item.id, label: `${item.sku} · ${item.name}` }))} /></Form.Item><Form.Item name="unitPrice" label="目錄單價（未稅，TWD）" rules={[{ required: true, message: '請填寫價格' }]}><InputNumber min={0} precision={2} style={{ width: '100%' }} /></Form.Item><Form.Item name="isPublished" label="對客戶發布" valuePropName="checked"><Switch /></Form.Item><Alert type="info" message="只發布且有有效價格的商品才會顯示在客戶前台。" /></Form></Modal>

    <Modal title="客戶專屬價格" open={priceOpen} confirmLoading={saving} onCancel={() => setPriceOpen(false)} onOk={() => void savePrice()} okText="儲存" destroyOnHidden><Form form={priceForm} layout="vertical"><Form.Item name="customerId" label="客戶" rules={[{ required: true, message: '請選擇客戶' }]}><Select showSearch optionFilterProp="label" options={setup?.customers.map((item) => ({ value: item.id, label: item.companyName || item.name }))} /></Form.Item><Form.Item name="productId" label="商品" rules={[{ required: true, message: '請選擇商品' }]}><Select showSearch optionFilterProp="label" options={setup?.products.map((item) => ({ value: item.id, label: `${item.sku} · ${item.name}` }))} /></Form.Item><Form.Item name="unitPrice" label="專屬單價（未稅，TWD）" rules={[{ required: true, message: '請填寫價格' }]}><InputNumber min={0} precision={2} style={{ width: '100%' }} /></Form.Item><Form.Item name="validUntil" label="有效至（選填）"><Input type="date" /></Form.Item><Form.Item name="isActive" label="啟用專屬價格" valuePropName="checked"><Switch /></Form.Item></Form></Modal>
