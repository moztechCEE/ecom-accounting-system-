#!/usr/bin/env python3
"""Prepare a DEV-only build on the reviewed, currently serving entry-audit images.
...
Examples (after merging the latest DEV source, reviewing, generating Prisma locally and committing):
...
BASE_SOURCE = '53188aabf1fc1e43fc8fdf3ec59aec791109338a'
BASES = {
    'backend': {
        'service': 'corely-erp-api-dev',
        'revision': 'corely-erp-api-dev-entry-audit-0923',
        'digest': 'sha256:63814da9ce650f20c2038303118c4eeda1398389e9f7d9191ff9ccc17f78ace0',
    },
    'frontend': {
        'service': 'corely-erp-dev',
        'revision': 'corely-erp-dev-entry-audit-0923',
        'digest': 'sha256:15784ad5c94e44461073b870cf74e6b84b9bfda516eacf020a67911037d77234',
    },
}
UNCHANGED_RUNTIME = [
...
def main():
    ...
    check(subprocess.run(['git', 'merge-base', '--is-ancestor', BASE_SOURCE, sha], cwd=ROOT,
                         stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode == 0,
    ...
    if args.context_dir:
        ...
        context = Path(tempfile.mkdtemp(prefix='erp-corely-claw-dev-'))
    steps, images = [], []
    for folder, expected in BASES.items():
        ...
        image = REGISTRY + expected['service'] + ':corely-claw-' + sha
        images.append(image)
    ...
    manifest = {
        'sourceSha': sha, 'requiredBaseSource': BASE_SOURCE, 'devOnly': True,
