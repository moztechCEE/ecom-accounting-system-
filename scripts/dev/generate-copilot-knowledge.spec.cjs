...
const { collectAppRoutes } = require('./generate-copilot-knowledge.cjs');
...
test('reviewed catalog passes bilingual, route, original-ID and hash checks without writes', (t) => {
  ...
  assert.match(result.output, new RegExp(`${manifest.entryCount} bilingual guides, ${manifest.groups.length} groups, ${manifest.coverage.routes.length} routes`));
  assert(manifest.coverage.routes.includes('/b2b/catalog'));
  assert(manifest.coverage.routes.includes('/b2b/requests'));
  assert(manifest.coverage.dynamicRoutes.includes('/b2b/requests/:id'));
  assert(!manifest.coverage.routes.includes('/catalog'));
  assert(!manifest.coverage.routes.includes('/requests'));
...
test('nested routes resolve under their parent, including pathless and index layouts', () => {
  const routes = collectAppRoutes(`
    <Route path="/b2b" element={<PortalLayout />}>
      <Route index element={<Navigate to="catalog" replace />} />
      <Route element={<PathlessLayout><span>ready</span></PathlessLayout>}>
        <Route path="catalog" element={<Catalog />} />
        <Route path="requests/:id" element={<Request />} />
      </Route>
    </Route>
    {/* <Route path="/commented-out" element={null} /> */}
  `);
  assert.deepEqual(routes, ['/b2b', '/b2b/catalog', '/b2b/requests/:id']);
});
test('a fake unprefixed alias cannot pass route validation', (t) => {
  const root = fixture(t);
  edit(root, (data) => { data.entries[0].aliases = [...(data.entries[0].aliases || []), '/catalog']; });
  const result = run(root, '--write');
  assert.equal(result.code, 1);
  assert.match(result.output, /route does not exist: \/catalog/);
});
test('a configured navigation destination must resolve to an App route', (t) => {
  const root = fixture(t);
  fs.appendFileSync(path.join(root, 'frontend/src/config/navigation.ts'), "\nconst extra = { key: '/not-a-route' };\n");
  const result = run(root, '--write');
  assert.equal(result.code, 1);
