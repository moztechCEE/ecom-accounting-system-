...
BASE_REVISIONS = {s: s + '-entry-audit-0923' for s in DEV}
BASE_IMAGES = {
    API: REGISTRY + API + '@sha256:63814da9ce650f20c2038303118c4eeda1398389e9f7d9191ff9ccc17f78ace0',
    WEB: REGISTRY + WEB + '@sha256:15784ad5c94e44461073b870cf74e6b84b9bfda516eacf020a67911037d77234',
}
...
def resolve_build(build_id, source):
    ...
    result = {}
    for name in DEV:
        expected = REGISTRY + name + ':corely-claw-' + source
    ...
    require(set(build.get('images', [])) ==
            {REGISTRY + s + ':corely-claw-' + source for s in DEV},
    ...
    for name in DEV:
        ...
        if previous:
            ...
            require(ready(revision) and revision['status'].get('imageDigest') == BASE_IMAGES[name],
                    name + ': original entry-audit revision/image changed')
        else:
            require(live[name]['status'].get('latestReadyRevisionName') == BASE_REVISIONS[name]
                    and image_of(live[name]) == BASE_IMAGES[name], name + ': reviewed entry-audit baseline changed')
    suffix = 'claw-' + args.source_sha[:8]
...
def planned_spec(phase, state, live):
    ...
    allowed = {'CORS_ORIGIN'} if name == API else {'API_URL', 'WS_URL'}
