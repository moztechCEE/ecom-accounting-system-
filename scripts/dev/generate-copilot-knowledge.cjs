...
function collectRoutes() {
  const app = read('frontend/src/App.tsx').replace(/\{\/\*[\s\S]*?\*\/\}/g, '');
  const staticRoutes = [...app.matchAll(/<Route\s+path="([^"]+)"/g)].map((m) => normalizeRoute(m[1]));
...
function build() {
  ...
  for (const entry of entries) {
    ...
    for (const route of [entry.path, ...(entry.aliases || [])].filter(Boolean)) {
      ...
      assert(routeInfo.staticRoutes.some((known) => known === normalized || (known === '/warehouse/:report' && /^\/warehouse\/[^/]+$/.test(normalized))), `${entry.id}: route does not exist: ${route}`);
      covered.add(route);
