...
2. 為特休新增年資級距規則
3. 把薪資單下載改成真正 PDF 產出，而不是目前的列印版
4. 補假別/額度/薪資批次操作的 audit log
5. 把 `paid` 後續流程接到真正銀行出款 / 對帳 / 匯款憑證管理
...
1. 先跑 migration
2. 驗證員工/部門建立與編輯流程
3. 驗證員工送假 -> 核准 -> 薪資計算是否能產生 `LEAVE_DEDUCTION`
4. 驗證薪資批次建立 -> 送審 -> 批准流程
5. 驗證員工端個人薪資單是否能正確只看到自己的資料
6. 驗證薪資過帳是否成功建立 JournalEntry
7. 驗證發薪後是否正確寫入 `paidAt / paidBy / bankAccountId`
8. 再補真正 PDF 薪資單、銀行出款對帳與更細的主管審核模型
