...
2026-05-11 使用者確認目前業績歸屬規則：Shopify 帳號對應 MOZTECH；Shopline 對應 BONSON；1Shop 團購平台會混合 MORITEK、BONSON、MOZTECH、AIRITY，需依商品 / SKU / 品牌欄位拆分，而不是把 `萬魔未來工學院` 這類平台名當品牌。
...
- `3290494` / `shopify-main` 的綠界電子發票 `HashKey` / `HashIV` 已放入 Secret Manager / Cloud Run env，不寫入 repo；readiness 已可辨識該帳號 ready，但 `ECPAY_EINVOICE_ISSUING_ENABLED=false` 仍會阻擋正式開票。
...
- Cloud Run 驗證：`shopify-main` 多筆發票查詢成功，`2026-05-01` 到 `2026-05-04` 總筆數 142、樣本回傳 5 筆；民國 115 年字軌配號查詢成功，回傳 3 組。
...
### 綠界服務矩陣：金流 / 物流 / 電子發票 / 電子收據 / MPOS

2026-05-04 補充：使用者已確認 `3290494` 與 `3150241` 兩個綠界帳號都具備金流 / MPOS、電子發票、物流、電子收據分類的介接資訊。金鑰本身不寫入 repo；已完成電子發票 secrets，其他 service 後續也應進 Secret Manager。

- 金流：
  - 一定會用，屬於訂單閉環核心。
  - 負責付款狀態、撥款、退款、實際金流手續費、銀行入帳核銷與現金流報表。
  - 應拆成 `shopify-main / 3290494` 與 `groupbuy-main / 3150241` 兩條對帳鏈，不可混用。
- 物流：
  - 若通路使用綠界物流、超商取貨付款、貨到付款或物流代收，就會用到。
  - 負責出貨狀態、物流費、代收款、在途應收、庫存出貨與訂單可關帳條件。
  - COD / 超商取貨付款不可只看訂單成立，需等物流代收或撥款資料回填後核銷。
- 電子發票：
  - 已完成雙帳號只讀查詢、字軌配號查詢與 readiness。
  - 正式開票仍需小額測試 `Issue / GetIssue / Invalid / Allowance` 後才可啟用。
- 電子收據：
  - 視公司實際使用場景導入；它不能取代電子發票。
  - 可用於不開發票但需要收據 / 憑證的交易或費用流程，但需先定義和電子發票、AP 憑證的邊界。
- MPOS：
  - 若有實體門市、展場、快閃、電話刷卡或現場刷卡，會納入營運中樞。
  - MPOS 收款應標記為實體 / 活動通路，建立 `offline/mpos` 類型的 Payment，再跟綠界撥款與銀行入帳核銷。
