...
- Shopline 已取得可用 OpenAPI access token，已掛入 Secret Manager / Cloud Run，白名單已加入 Cloud Run 固定出口 IP；Cloud Run `token-info` 已確認可讀到 BONSON 店鋪 merchant / handle。一般 OpenAPI 可取回的 `2024-05-05` 到 `2026-05-05` 訂單 / 顧客已正式同步進 Cloud SQL：SHOPLINE `SalesOrder` 4689 筆、訂單總額 9,250,001，Payment 4687 筆。2026-05-06 已補上 SHOPLINE Payments Admin OpenAPI connector，可查 `balance_transactions.json`、`transactions.json`、`payouts.json`、`balance.json`，並可把帳務明細匯入 `shoplinepay` 對帳流程；仍需 Cloud Run 實測 token 是否有 `read_payment` 權限。Momo / PChome / Pinkoi / LINE 禮物等其他通路目前尚未形成實際訂單資料流。
...
- Shopline health：Cloud Run `connection-info` 已可讀到 Shopline 設定；修正後的 OpenAPI token 已通過 Cloud Run `token-info` 驗證，merchant handle 為 `onemorefuture`，merchant id 已確認為 `5e0738e792f5c90009548b54`。已新增只讀 `agents`、`preview/orders`、`preview/customers` 診斷端點；Cloud Run 呼叫成功。`sync/orders`、`sync/customers` 已完成 2024-05-05 至 2026-05-05 回補，且 `ecom-accounting-shopline-auto-sync` 已建立，每 20 分鐘增量同步。2026-05-06 已新增 Payments API 預覽與 `sync/payments/billing-records`，剩餘缺口是 Cloud Run read_payment 實測、兩年以上 archived orders、webhook topic 與簽章驗證、商品 / 分類 / 庫存、Shopline invoice 正式回寫。本機直連 Shopline 仍會因 IP 白名單被擋，應以 Cloud Run 固定出口驗證為準。
- 2026-05-06 補充：會計工作台已新增「同步 Shopline Payments 帳務」入口，會呼叫 `POST /integrations/shopline/sync/payments/billing-records`；串接準備也改為明確提示 `read_payment` 與 `SHOPLINE_PAYMENTS_SYNC_ENABLED`，避免使用者以為 Shopline Payment 仍只能靠 Excel 匯入。
- 2026-05-07 實測：Cloud Run `payments/balance` 與 `payments/billing-records` 仍回 HTML / redirect 型錯誤，不是有效 JSON；代表一般 Shopline OpenAPI 可用，但 Payments Admin OpenAPI 尚未確認打通。後端已新增 `GET /integrations/shopline/payments/readiness`，並讓 Admin request 遇到 3xx redirect / HTML 時回傳明確診斷，下一次部署後可直接判斷是 `SHOPLINE_ADMIN_API_BASE_URL`、handle、version、`read_payment` 權限或 Shopline Payments OpenAPI 未啟用。
- 2026-05-08 部署後重測：`GET /integrations/shopline/payments/readiness` 已可回清楚診斷。店鋪 `BONSON_SHOPLINE` / handle `onemorefuture` / merchant id `5e0738e792f5c90009548b54` 設定完整，但 Payments Admin base `https://onemorefuture.myshopline.com/admin/openapi/v20260301` 會 `302` redirect 到 `https://www.shopline.com`。因此目前卡點不是一般 OpenAPI token，而是 Payments Admin API host / version / `read_payment` / Shopline Payments OpenAPI 啟用狀態需由 Shopline 後台或官方確認。
