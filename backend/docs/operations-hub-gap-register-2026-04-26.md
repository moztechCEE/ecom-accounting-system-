...
- `frontend/src/pages/DashboardPage.tsx`：CEO Dashboard 紅燈警示與 KPI 會顯示超收 / 疑似重複收款筆數與差額，並導向應收帳款頁處理明細；當 Dashboard 選「全部」沒有日期邊界時，AR 風險監控會改用最近 90 天，避免正式環境無界 `/ar/monitor` 查詢失敗造成漏報。
