...
- Shopline 已取得 access token，已掛入 Secret Manager / Cloud Run，白名單已加入 Cloud Run 固定出口 IP；但 `token-info` 仍回 `UnsupportedAccessTokenError`，需重新確認 token 類型、API 權限或正式 handle code。Momo / PChome / Pinkoi / LINE 禮物等其他通路目前尚未形成實際訂單資料流。

### B-2. 實際整合健康狀態

- Shopify health：成功，connection successful
- 1Shop health：503 Service Unavailable
- Shopline health：Cloud Run `connection-info` 已可讀到 Shopline 設定；以推測 handle `onemorefuture` 測試 `token-info` 時，Shopline 回 `UnsupportedAccessTokenError`。正式 orders / customers / transactions 同步仍需 Shopline 提供可用 OpenAPI token / 正式 `User-Agent / handle code`。
