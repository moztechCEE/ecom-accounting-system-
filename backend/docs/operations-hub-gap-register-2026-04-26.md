...
- 2026-05-06 實測：Cloud Run `payments/balance` 與 `payments/billing-records` 目前仍未通，log 顯示 Shopline Payments Admin endpoint 回傳 HTML 而非 JSON，舊 adapter 因此變成不明確 500。本地已修正 adapter：Admin request 帶入 `User-Agent` handle，並把非 JSON 回應包成明確錯誤；需部署後再重測，判斷是 `read_payment`、Admin API host/version、handle，或 Shopline 後台權限問題。
