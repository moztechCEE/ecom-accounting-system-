...
- Shopline 已取得 access token，但尚未補 handle code / 多店設定，也尚未掛入 Cloud Run secret；Momo / PChome / Pinkoi / LINE 禮物等其他通路目前尚未形成實際訂單資料流。

### B-2. 實際整合健康狀態

- Shopify health：成功，connection successful
- 1Shop health：503 Service Unavailable
- Shopline health：已修正 adapter，可在 token-only 階段先查 token-info；正式 orders / customers / transactions 同步仍需 Shopline `User-Agent / handle code` 與 Cloud Run secret/env 設定。
