...
2026-09-11 儲運管理總覽與四項查核工具（本機增量，未部署）：

- 管理員直接看揀貨／裝箱兩佇列、進度、人員與未結例外；作業角色只有揀貨／裝箱，不另設出貨人員。員工維持授權工作站與個人資訊，管理工具需個別 permission。導覽不因巢狀報表跳回全模組。
- 新增操作日誌、例外總覽、刷錯分析、新品不良分析原生頁面與 API 契約。來源以只讀快照和已授權公司／品牌／訂單 mapping 計算全量篩選統計，不把前 1,000 筆當完整資料。例外預設全部期間未結案；刷錯與新品回報不誤稱比率。
- 持久命令的錯碼拒絕改為保存單一刷錯事件／receipt 後回拒絕，重送不重算、不加核對數量；仍限隔離 staging DB，正式 command gate 未開。
- ERP build、43 suites / 181 tests，frontend build 與指定 15 tests，WMS 6 tests、既有跨 repo HTTP 4 tests 通過。瀏覽器驗證四報表、角色拒絕、搜尋分頁、SN 明細及大字工作站。不是正式資料／硬體驗收。
- 本輪未套 migration、賦權、建金鑰、push、部署、改 IAM 或正式流量。無時區的例外／新品紀錄需先核對 legacy 寫入時區；未配置時報表 fail closed。新 permission catalog 不會自動賦權。
- 核可／結案、新品更換寫入、真實歷史對照／搬遷、每日排班、封箱標籤／物流交接与原流程完整 parity 仍未完成。詳見 `warehouse-management-2026-09-11.md`，不可宣稱整套 WMS 已上線整合。

2026-09-11 主管分析歸位 ERP 與狀態卡片修正（本機增量，未部署）：

- 依使用者更正，四項分析集中在 ERP 儲運管理中心側欄，移除工作台底部重複入口；主管路由保留 ERP 的其他授權模組，不再自動限縮成作業人員工作區。
- 主管總覽移除揀貨／裝箱／拋單按鈕；有操作授權者另由工作站入口作業。純揀貨／裝箱人員仍只有自己的授權工作與個人資訊，直接開主管報表被拒絕。
- 四狀態改為獨立圓角、低彩度琥珀／藍／綠／紫卡片。新增唯讀主管測試角色；沒有賦予真實主管權限或修改後端金流／庫存／物流行為。
- 本次不部署、不推送、不改 IAM，不修改 WMS 或售後來源。私有 staging 與真實資料連接、mapping、歷史時區等驗收缺口保持未完成。
- 驗證：frontend build、16 tests、diff check 通過；主管四報表、管理員導覽往返、揀貨員直接開報表拒絕及 390px 無水平溢出完成本機瀏覽器檢查。

2026-09-11 儲運主管介面正式發布準備（僅 frontend）：
