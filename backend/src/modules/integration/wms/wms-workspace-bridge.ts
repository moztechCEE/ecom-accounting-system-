import { ForbiddenException, ServiceUnavailableException, BadRequestException, NotFoundException } from '@nestjs/common';
...
export function projectWorkspaceResponse(value:unknown,detail:boolean,expectedId?:string) {
  const data=record(value);
  if(data.source!=='wms'||data.contractVersion!=='wms.workspace-read.v1')throw invalid();
  if(!detail){
    if(data.mode!=='read_only'||!Array.isArray(data.items)||data.items.length>100)throw invalid();
    return {items:data.items.map(row),total:count(data.total),mode:'read_only',source:'wms'};
  }
  if(data.id!==expectedId||!Array.isArray(data.items)||data.items.length>1000||!Array.isArray(data.allowedActions)||data.allowedActions.length!==0)throw invalid();
  ...
  // A read snapshot is never a command revision or permission grant.
...
export class WmsWorkspaceBridge {
  ...
  async read(actorId:string,query:Query,id?:string) {
    ...
    const url=new URL('/api/integrations/erp/v1/orders'+(id?'/'+encodeURIComponent(id):''),base);
    for(const key of ['view','search','page','pageSize'] as const)if(query[key]!==undefined)url.searchParams.set(key,String(query[key]));
    const token=await new JwtService().signAsync({entityId:query.entityId,station:area,scope:'wms.workspace.read'},
    ...
    let result:unknown;
    try {
      const response=await this.fetcher(url,{method:'GET',headers:{Authorization:`Bearer ${token}`,Accept:'application/json'},redirect:'error',signal:AbortSignal.timeout(5000)});
    ...
    }catch(e){if(e instanceof ForbiddenException||e instanceof NotFoundException)throw e;throw new ServiceUnavailableException({code:'WMS_SOURCE_UNAVAILABLE',message:'WMS 連線未完成，請稍後重試'});}
    return projectWorkspaceResponse(result,!!id,id);
  }
}
