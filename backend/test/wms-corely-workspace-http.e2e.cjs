...
function fixture(duplicateBarcode = false) {
  ...
  const item = { id: 11, order_id: 8, source_platform: 'Corely', source_line_id: 'line-1', product_code: 'SKU',
    product_name: 'Synthetic item', barcode: '1234', quantity: 1, picked_quantity: 0, packed_quantity: 0 };
  const items = duplicateBarcode ? [item, { ...item, id: 12, source_line_id: 'line-2' }] : [item];
  ...
  const pool = { async query(sql, params = []) {
    ...
    if (sql.startsWith('SELECT * FROM order_items WHERE')) return { rows: items.map(row => ({ ...row })) };
    if (sql.startsWith('SELECT id FROM order_items WHERE order_id='))
      return { rows: items.filter(row => row.source_line_id === params[1]).map(row => ({ id: row.id })) };
    ...
    if (sql.includes('SELECT i.erp_order_id,i.payload,o.status')) {
      ...
      return { rows: ready ? [{ erp_order_id: 'sale-1', payload: sourceOrder.payload, status: order.status,
        ...
        picker_name: order.picker_id === 7 ? 'Picker' : null,
        packer_name: order.packer_id === 9 ? 'Packer' : null,
        required: items.length, picked: items.reduce((n, row) => n + row.picked_quantity, 0),
        packed: items.reduce((n, row) => n + row.packed_quantity, 0) }] : [] };
    }
  ...
  return { order, item, items, pool, queries };
}
...
test('duplicate barcode stays ambiguous until ERP sends the selected Corely source line', { timeout: 30000 }, async () => {
  const { order, items, pool } = fixture(true);
  const keys = generateKeyPairSync('rsa', { modulusLength: 2048 });
  const env = { ERP_WORKSPACE_READ_ENABLED: 'true', ERP_WORKSPACE_COMMANDS_ENABLED: 'true',
    ERP_WORKSPACE_PUBLIC_KEY: keys.publicKey.export({ type: 'spki', format: 'pem' }).toString(),
    ERP_WORKSPACE_ISSUER: 'erp-test', ERP_WORKSPACE_AUDIENCE: 'wms-test' };
  ...
  const claimHandler = async (req, res) => {
    await req.workspaceGuard(pool, 8);
    order.status = 'picking'; order.picker_id = 7;
    res.json({ outcome: 'claimed' });
  };
  const scanHandler = async (req, res) => {
    await req.workspaceGuard(pool, 8);
    if (!req.body.orderItemId) return res.status(409).json({ code: 'SOURCE_ITEM_REQUIRED' });
    seenNativeIds.push(req.body.orderItemId);
    const item = items.find(row => row.id === req.body.orderItemId);
    assert.ok(item);
    item.picked_quantity = 1;
    if (items.every(row => row.picked_quantity === 1)) order.status = 'picked';
    res.json({ format: 'delta-v1' });
  };
  const app = express(); app.use(express.json());
  app.use('/api/integrations/erp/v1', createCorelyWorkspaceReadRouter({ pool, env }));
  ...
  try {
    const config = { NODE_ENV: 'test', WMS_WORKSPACE_READ_ENABLED: 'true', WMS_WORKSPACE_COMMANDS_ENABLED: 'true',
      WMS_WORKSPACE_URL: `http://127.0.0.1:${server.address().port}/`,
      WMS_WORKSPACE_ISSUER: 'erp-test', WMS_WORKSPACE_AUDIENCE: 'wms-test',
      WMS_WORKSPACE_PRIVATE_KEY: keys.privateKey.export({ type: 'pkcs8', format: 'pem' }).toString() };
    const bridge = new WmsWorkspaceBridge(erpPermissions('erp-picker'), config);
    let current = await bridge.read('erp-picker', { entityId: 'company', area: 'pick' }, 'sale-1');
    assert.deepEqual(current.items.map(row => row.id), ['line-1', 'line-2']);
    current = await bridge.command('erp-picker', 'company', 'sale-1', 'pick', 'claim',
      { entityId: 'company', expectedRevision: current.revision, requestId: 'claim' });
    const scan = async (requestId, itemId) => bridge.command('erp-picker', 'company', 'sale-1', 'pick', 'scan',
      { entityId: 'company', expectedRevision: current.revision, requestId, scanValue: '1234',
        ...(itemId ? { itemId } : {}) });
    await assert.rejects(scan('ambiguous', undefined), /WMS_REVISION_OR_REQUEST_CONFLICT/);
    assert.deepEqual(seenNativeIds, []);
    current = await scan('selected-first', 'line-1');
    assert.equal(current.picked, 1);
    // The source barcode remains ambiguous even after one line is full.
    await assert.rejects(scan('still-ambiguous', undefined), /WMS_REVISION_OR_REQUEST_CONFLICT/);
    current = await scan('selected-second', 'line-2');
    assert.equal(current.state, 'picked');
    assert.deepEqual(seenNativeIds, [11, 12]);
  } finally { await new Promise(resolve => server.close(resolve)); }
});
